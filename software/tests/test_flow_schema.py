...
4. 端口规范 v2 验证（CONDITION source/neq, RECORD counter, EXECUTE 简化）
...
NODE_SCHEMAS = {
    ...
    "delay": {
        ...
        "help": "流程执行到此节点时暂停指定时长（0.1秒 ~ 1小时）。1 秒 = 1 秒。",
        "ports": {"inputs": -1, "outputs": 1},
    },
    "condition": {
        "label": "条件判断",
        ...
        "help": "根据上游数据做条件判断。source 选择数据来源，operator 选择比较方式，value 设置阈值。",
    ...
    "record": {
        ...
        "fields": [
            ...
            {"key": "counter_name", "label": "计数器名称", "type": "text", "maxLength": 64, "required": False},
            {
                "key": "counter_op",
                "label": "计数器操作",
                "type": "select",
...
class TestSchemaCompleteness:
    ...
    def test_all_node_types_exist(self):
        ...
        expected_types = {'start', 'end', 'trigger', 'delay', 'condition',
                          'execute', 'loop', 'and', 'not', 'record', 'record_end'}
...
class TestFunctionalNodeSchemas:
    ...
    def test_delay_schema(self):
        """DELAY: duration_s 浮点数，0.1~3600（秒）。"""
        schema = NODE_SCHEMAS['delay']
        assert len(schema['fields']) == 1
        field = schema['fields'][0]
        assert field['key'] == 'duration_s'
        assert field['type'] == 'number'
        assert field['min'] == 0.1
        assert field['max'] == 3600
        assert field['default'] == 1
        assert field['required'] is True

    def test_condition_schema(self):
        """CONDITION: source + operator + value（3 字段）。"""
        schema = NODE_SCHEMAS['condition']
        assert len(schema['fields']) == 3
        assert schema['fields'][0]['key'] == 'source'
        assert schema['fields'][0]['default'] == 'trigger_count'
        assert schema['fields'][1]['key'] == 'operator'
        assert schema['fields'][1]['default'] == 'gt'
        assert schema['fields'][2]['key'] == 'value'
        assert schema['fields'][2]['type'] == 'number'
        assert schema['fields'][2]['min'] == 0
        assert schema['fields'][2]['max'] == 999999
    ...
    def test_record_schema(self):
        """RECORD: event_name + counter_name + counter_op（3 字段，无 experiment_type）。"""
        schema = NODE_SCHEMAS['record']
        assert len(schema['fields']) == 3
        assert schema['fields'][0]['key'] == 'event_name'
        assert schema['fields'][1]['key'] == 'counter_name'
        assert schema['fields'][1]['required'] is False
        assert schema['fields'][2]['key'] == 'counter_op'
        assert schema['fields'][2]['default'] == '+1'
...
class TestParameterValidation:
    ...
    def test_delay_duration_too_low(self):
        """DELAY duration_s < 0.1 -> 校验失败。"""
        valid, errors = validate_node_params('delay', {'duration_s': 0})
        assert valid is False
        assert any('不能小于 0.1' in e for e in errors)

    def test_delay_duration_too_high(self):
        """DELAY duration_s > 3600 -> 校验失败。"""
        valid, errors = validate_node_params('delay', {'duration_s': 3601})
        assert valid is False
        assert any('不能超过 3600' in e for e in errors)

    def test_delay_duration_valid(self):
        """DELAY duration_s = 1 -> 校验通过。"""
        valid, errors = validate_node_params('delay', {'duration_s': 1})
        assert valid is True
    ...
    def test_record_with_counter(self):
        """RECORD 含 counter 参数 -> 校验通过。"""
...
class TestSaveLoadConsistency:
    ...
    def test_record_counter_roundtrip(self):
        """RECORD counter 参数保存/加载一致。"""
        node = self._build_flow_node('record', {
            'event_name': '压杆记录',
            'counter_name': 'press_count',
            'counter_op': '+1',
        })
        json_str = json.dumps(node)
        restored = json.loads(json_str)
        assert restored['params']['counter_name'] == 'press_count'
        assert restored['params']['counter_op'] == '+1'
...
class TestSchemaVsSpec:
    ...
    def test_delay_duration_range(self):
        """DELAY duration_s: 0.1 ~ 3,600。"""
        field = NODE_SCHEMAS['delay']['fields'][0]
        assert field['min'] == 0.1 and field['max'] == 3600
    ...
    def test_condition_operator_has_neq(self):
        ...
        options = [opt['value'] for opt in NODE_SCHEMAS['condition']['fields'][1]['options']]
        assert set(options) == {'eq', 'neq', 'gt', 'lt', 'gte', 'lte'}

    def test_condition_operator_default(self):
        """CONDITION operator 默认值 gt。"""
        field = NODE_SCHEMAS['condition']['fields'][1]
        assert field['default'] == 'gt'

    def test_condition_value_range(self):
        """CONDITION value: 0 ~ 999,999。"""
        field = NODE_SCHEMAS['condition']['fields'][2]
        assert field['min'] == 0 and field['max'] == 999999
    ...
    def test_record_has_counter_fields(self):
        """RECORD 有 counter_name + counter_op 字段。"""
        keys = [f['key'] for f in NODE_SCHEMAS['record']['fields']]
        assert 'counter_name' in keys
        assert 'counter_op' in keys
