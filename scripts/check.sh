...
#
# 用法:
#   ./check.sh                  # 检查默认端口 8000（G2 阶段收尾统一）
#   ./check.sh 8001             # 检查指定端口
#   ./scripts/manage.sh check   # 通过 manage.sh 调用
#
# 检查项:
#   1. 服务进程是否运行
#   2. HTTP GET / 返回 200
...
#
# 修复指引:
#   [FAIL] 服务未运行   → ./scripts/manage.sh start 启动
...
if lsof -ti:"$PORT" 2>/dev/null | grep -q .; then
  ...
  check "服务运行中 (端口 ${PORT})" "FAIL" "请运行 ./scripts/manage.sh start ${PORT} 启动服务"
fi
...
if [ -d "${PROJECT_ROOT}/software/data_store/experiments" ]; then
